<?php
	 ...
	 * @param array        $properties a mapping from object class names to the properties that need to put into the
	 *                                 resulting arrays. The properties specified for each class is an array of the
	 *                                 following format:
	 *
	 * ~~~
	 * [
	 ...
	 */
	function arrayToArray( $object, $properties = array(), $recursive = true ) {
		if ( is_array( $object ) ) {
			if ( $recursive ) {
				foreach ( $object as $key => $value ) {
					if ( is_array( $value ) || is_object( $value ) ) {
						$object[ $key ] = arrayToArray( $value, $properties, true );
					}
				}
			}
				...
				if ( ! empty( $properties[ $className ] ) ) {
					...
					foreach ( $properties[ $className ] as $key => $name ) {
						if ( is_int( $key ) ) {
							...
							$result[ $key ] = arrayGetValue( $object, $name );
						}
					}

					return $recursive ? arrayToArray( $result ) : $result;
				}
			}
	 ...
	 * $fullName = getValue($user, function ($user, $defaultValue) {
	 *     return $user->firstName . ' ' . $user->lastName;
	 * });
	 * // using dot format to retrieve the property of embedded object
	 * $street = getValue($users, 'address.street');
	 * ~~~
	 *
	...
	function arrayGetValue( $array, $key, $default = null ) {
		...
		if ( ( $pos = strrpos( $key, '.' ) ) !== false ) {
			$array = arrayGetValue( $array, substr( $key, 0, $pos ), $default );
	...
	function arrayIndex( $array, $key ) {
		...
		foreach ( $array as $element ) {
			$value            = arrayGetValue( $element, $key );
	 ...
	 */
	function arrayGetColumn( $array, $name, $keepKeys = true ) {
		$result = array();
		if ( $keepKeys ) {
			foreach ( $array as $k => $element ) {
				$result[ $k ] = arrayGetValue( $element, $name );
			}
		} else {
			foreach ( $array as $element ) {
				$result[ ] = arrayGetValue( $element, $name );
			}
		}
	...
	function arrayMap( $array, $from, $to, $group = null ) {
		$result = array();
		foreach ( $array as $element ) {
			$key   = arrayGetValue( $element, $from );
			$value = arrayGetValue( $element, $to );
			if ( $group !== null ) {
				$result[ arrayGetValue( $element, $group ) ][ $key ] = $value;
			} else {
				$result[ $key ] = $value;
			}
		}
	 ...
	 */
	function arrayKeyExists( $key, $array, $caseSensitive = true ) {
		if ( $caseSensitive ) {
	 ...
	 * @throws Exception
	 */
	function arrayMultisort( &$array, $key, $direction = SORT_ASC, $sortFlag = SORT_REGULAR ) {
	 ...
	 */
	function arrayHtmlEncode( $data, $valuesOnly = true, $charset = null ) {
		if ( $charset === null ) {
		...
		foreach ( $data as $key => $value ) {
			...
			if ( is_string( $value ) ) {
				...
				$d[ $key ] = arrayHtmlEncode( $value, $charset );
			}
		}
	 ...
	 */
	function arrayHtmlDecode( $data, $valuesOnly = true ) {
		$d = array();
		foreach ( $data as $key => $value ) {
			if ( ! $valuesOnly && is_string( $key ) ) {
	 ...
	 */
	function arrayIsAssociative( $array, $allStrings = true ) {
	 ...
	 * @return boolean whether the array is associative
	 */
	function arrayIsIndexed( $array, $consecutive = false ) {
		if ( ! is_array( $array ) ) {
			return false;
		}
	...
	function arrayAppend( Array $array ) {
		$args = func_get_args();
		array_shift( $args );
		if ( empty( $args ) ) {
			return $array;
	...
	function arrayGetFirst( $array ) {
		$first = array_values( $array );

		return isset( $first[ 0 ] ) ? $first[ 0 ] : false;
	}
