<?php
	 ...
	 * @changes    1.0.0b13  Updated documentation about symmetric-key encryption to explicitly state block and key
	 *             sizes, added base64 type to ::randomString() [wb, 2010-11-06]
	 * @changes    1.0.0b12  Fixed an inline comment that incorrectly references AES-256 [wb, 2010-11-04]
	 * @changes    1.0.0b11  Updated class to use Phoenix_Framework_Flourish_Core::startErrorCapture() instead of
	 *             `error_reporting()` [wb, 2010-08-09]
	 * @changes    1.0.0b10  Added a missing parameter to an Phoenix_Framework_Exception in ::randomString()
	 *             [wb, 2010-07-29]
	 * @changes    1.0.0b9   Added ::hashHMAC() [wb, 2010-04-20]
	 * @changes    1.0.0b8   Fixed ::seedRandom() to pass a directory instead of a file to
	 *             [http://php.net/disk_free_space `disk_free_space()`] [wb, 2010-03-09]
	 * @changes    1.0.0b7   SECURITY FIX: fixed issue with ::random() and ::randomString() not producing random output
	 *             on OSX, made ::seedRandom() more robust [wb, 2009-10-06]
	 * @changes    1.0.0b6   Changed ::symmetricKeyEncrypt() to throw an Phoenix_Framework_Exception when the
	 *             $secret_key is less than 8 characters [wb, 2009-09-30]
	 * @changes    1.0.0b5   Fixed a bug where some windows machines would throw an
	 *             Phoenix_Framework_Exception when generating random strings or numbers [wb, 2009-06-09]
	 * @changes    1.0.0b4   Updated for new Phoenix_Framework_Flourish_Core API [wb, 2009-02-16]
	 * @changes    1.0.0b3   Changed @ error suppression operator to `error_reporting()` calls [wb, 2009-01-26]
	 * @changes    1.0.0b2   Backwards compatibility break - changed ::symmetricKeyEncrypt() to not encrypt the IV
	 *             since we are using HMAC on it [wb, 2009-01-26]
	 * @changes    1.0.0b    The initial implementation [wb, 2007-11-27]
	 */
	class Phoenix_Framework_Flourish_Cryptography {
		// The following constants allow for nice looking callbacks to static methods
		const checkPasswordHash = 'Phoenix_Framework_Flourish_Cryptography::checkPasswordHash';
		const hashHMAC = 'Phoenix_Framework_Flourish_Cryptography::hashHMAC';
		const hashPassword = 'Phoenix_Framework_Flourish_Cryptography::hashPassword';
		const publicKeyDecrypt = 'Phoenix_Framework_Flourish_Cryptography::publicKeyDecrypt';
		const publicKeyEncrypt = 'Phoenix_Framework_Flourish_Cryptography::publicKeyEncrypt';
		const publicKeySign = 'Phoenix_Framework_Flourish_Cryptography::publicKeySign';
		const publicKeyVerify = 'Phoenix_Framework_Flourish_Cryptography::publicKeyVerify';
		const random = 'Phoenix_Framework_Flourish_Cryptography::random';
		const randomString = 'Phoenix_Framework_Flourish_Cryptography::randomString';
		const symmetricKeyDecrypt = 'Phoenix_Framework_Flourish_Cryptography::symmetricKeyDecrypt';
		const symmetricKeyEncrypt = 'Phoenix_Framework_Flourish_Cryptography::symmetricKeyEncrypt';
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When the private key is invalid
		 *
		...
		static private function hashWithSalt( $source, $salt ) {
			...
			return 'Phoenix_Framework_Flourish_Cryptography::password_hash#' . $salt . '#' . $sha1;
		}
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When the ciphertext appears to be corrupted
		 *
		...
		static public function publicKeyDecrypt( $ciphertext, $private_key_file, $password ) {
			...
			if ( sizeof( $elements ) != 4 || $elements[ 0 ] != 'Phoenix_Framework_Flourish_Cryptography::public' ) {
				throw new Phoenix_Framework_Exception(
		 ...
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return string  A base-64 encoded result containing a Flourish fingerprint and suitable for decryption using
		 *                 ::publicKeyDecrypt()
		 */
		static public function publicKeyEncrypt( $plaintext, $public_key_file ) {
			...
			return 'Phoenix_Framework_Flourish_Cryptography::public#' . base64_encode( $encrypted_keys[ 0 ] ) . '#' . base64_encode( $ciphertext ) . '#' . $hmac;
		}
		 ...
		 * @param  string  $type   The type of string to return: `'base64'`, `'base56'`, `'base36'`, `'alphanumeric'`,
		 *                         `'alpha'`, `'numeric'`, or `'hexadecimal'`, if a different string is provided, it
		 *                         will be used for the alphabet
		 *
		 * @throws Phoenix_Framework_Exception
		...
		static private function seedRandom() {
			...
			if ( ! Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) && $handle = fopen( '/dev/urandom', 'rb' ) ) {
				$bytes = fread( $handle, 4 );
				fclose( $handle );
				// On windows we should be able to use the Cryptographic Application Programming Interface COM object
			} elseif ( Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) && class_exists( 'COM', false ) ) {
				try {
		...
		static public function symmetricKeyDecrypt( $ciphertext, $secret_key ) {
			...
			$elements = explode( '#', $ciphertext );
			// We need to make sure this ciphertext came from here, otherwise we are gonna have issues decrypting it
			if ( sizeof( $elements ) != 4 || $elements[ 0 ] != 'Phoenix_Framework_Flourish_Cryptography::symmetric' ) {
				throw new Phoenix_Framework_Exception(
					'The ciphertext provided does not appear to have been encrypted using %s',
		 ...
		 * encryption and decryption.
		 *
		 * @throws Phoenix_Framework_Exception  When the $secret_key is less than 8 characters long
		 *
		 * @param  string $plaintext  The content to be encrypted
		 * @param  string $secret_key The secret key to use for encryption - must be at least 8 characters
		 *
		 * @return string  An encrypted and base-64 encoded result containing a Flourish fingerprint and suitable for
		 *                 decryption using ::symmetricKeyDecrypt()
		 */
		static public function symmetricKeyEncrypt( $plaintext, $secret_key ) {
			...
			return 'Phoenix_Framework_Flourish_Cryptography::symmetric#' . $encoded_iv . '#' . $encoded_ciphertext . '#' . $hmac;
		}
		 ...
		 *
		 * @return \Phoenix_Framework_Flourish_Cryptography
		 */
