<?php
	 ...
	 * @changes    1.0.0b39  Backwards Compatibility Break - ::output() now automatically ends any open output
	 *             buffering and discards the contents [wb, 2011-08-24]
	 ...
	 * @changes    1.0.0b35  Added calls to clearstatcache() in ::append() and ::write() to prevent incorrect data from
	 *             being returned by ::getMTime() and ::getSize() [wb, 2010-11-27]
	 * @changes    1.0.0b34  Added ::getExtension() [wb, 2010-05-10]
	 * @changes    1.0.0b33  Fixed another situation where ::rename() with the same name would cause the file to be
	 *             deleted [wb, 2010-04-13]
	 ...
	 * @changes    1.0.0b30  Changed the way files deleted in a filesystem transaction are handled, including
	 *             improvements to the Phoenix_Framework_Exception that is thrown [wb+wb-imarc, 2010-03-05]
	 * @changes    1.0.0b29  Fixed a couple of undefined variable errors in ::determineMimeTypeByContents() [wb,
	 *             2010-03-03]
	 * @changes    1.0.0b28  Added support for some JPEG files created by Photoshop [wb, 2009-12-16]
	 * @changes    1.0.0b27  Backwards Compatibility Break - renamed ::getFilename() to ::getName(), ::getFilesize() to
	 *             ::getSize(), ::getDirectory() to ::getParent(), added ::move() [wb, 2009-12-16]
	 * @changes    1.0.0b26  ::getDirectory(), ::getFilename() and ::getPath() now all work even if the file has been
	 *             deleted [wb, 2009-10-22]
	 * @changes    1.0.0b25  Fixed ::__construct() to throw an Phoenix_Framework_Exception when the file does not exist [wb, 2009-08-21]
	 * @changes    1.0.0b24  Fixed a bug where deleting a file would prevent any future operations in the same script
	 *             execution on a file or directory with the same path [wb, 2009-08-20]
	 * @changes    1.0.0b23  Added the ability to skip checks in ::__construct() for better performance in conjunction
	 *             with Phoenix_Framework_Flourish_Filesystem::createObject() [wb, 2009-08-06]
	 * @changes    1.0.0b22  Fixed ::__toString() to never throw an Phoenix_Framework_Exception [wb, 2009-08-06]
	 * @changes    1.0.0b21  Fixed a bug in ::determineMimeType() [wb, 2009-07-21]
	 * @changes    1.0.0b20  Fixed the Phoenix_Framework_Exception message thrown by ::output() when output buffering is turned on [wb,
	 *             2009-06-26]
	 * @changes    1.0.0b19  ::rename() will now rename the file in its current directory if the new filename has no
	 *             directory separator [wb, 2009-05-04]
	 * @changes    1.0.0b18  Changed ::__sleep() to not reset the iterator since it can cause side-effects [wb,
	 *             2009-05-04]
	 * @changes    1.0.0b17  Added ::__sleep() and ::__wakeup() for proper serialization with the filesystem map [wb,
	 *             2009-05-03]
	 * @changes    1.0.0b16  ::output() now accepts `TRUE` in the second parameter to use the current filename as the
	 *             attachment filename [wb, 2009-03-23]
	 * @changes    1.0.0b15  Added support for mime type detection of MP3s based on the MPEG-2 (as opposed to MPEG-1)
	 *             standard [wb, 2009-03-23]
	 ...
	 * @changes    1.0.0b12  Backwards compatibility break - Changed the second parameter of ::output() from
	 *             `$ignore_output_buffer` to `$filename` [wb, 2009-03-05]
	 * @changes    1.0.0b11  Changed ::__clone() and ::duplicate() to copy file permissions to the new file [wb,
	 *             2009-01-05]
	 * @changes    1.0.0b10  Fixed ::duplicate() so an Phoenix_Framework_Exception is not thrown when no parameters are passed [wb,
	 *             2009-01-05]
	 ...
	 * @changes    1.0.0b6   Added the ::__clone() method that duplicates the file on the filesystem when cloned [wb,
	 *             2008-12-11]
	 ...
	 */
	class Phoenix_Framework_Flourish_File implements Iterator, Countable {
		// The following constants allow for nice looking callbacks to static methods
		const create = 'Phoenix_Framework_Flourish_File::create';
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When no file was specified or the file already exists
		 *
		 ...
		 *
		 * @return Phoenix_Framework_Flourish_File
		 */
		static public function create( $file_path, $contents ) {
			...
			$directory = Phoenix_Framework_Flourish_Filesystem::getPathInfo( $file_path, 'dirname' );
			...
			$file = new Phoenix_Framework_Flourish_File( $file_path );
			if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::recordCreate( $file );
			}
		 ...
		 *
		 * @param  string $file     The file to check the mime type for - must be a valid filesystem path if no
		 *                          `$contents` are provided, otherwise just a filename
		 * @param  string $contents The first 4096 bytes of the file content - the `$file` parameter only need be a
		 *                          filename if this is provided
		 *
		 * @throws Phoenix_Framework_Exception
		...
		static public function determineMimeType( $file, $contents = null ) {
			...
			$extension = strtolower( Phoenix_Framework_Flourish_Filesystem::getPathInfo( $file, 'extension' ) );
		 ...
		 * @param  string $extension The extension of the filetype, only used for difficult files such as Microsoft
		 *                           office documents
		 *
		 * @return string  The mime type of the file
		 */
		static private function determineMimeTypeByContents( $content, $extension ) {
			...
			if ( $_0_2 == 'BM' && $length > 14 && in_array( $content[ 14 ], array(
						"\x0C",
						"\x28",
						"\x40",
						"\x80"
		 ...
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return \Phoenix_Framework_Flourish_File The new Phoenix_Framework_Flourish_File object
		 */
		public function __clone() {
			...
			$file = Phoenix_Framework_Flourish_Filesystem::makeUniqueName( $directory->getPath() . $this->getName() );
			copy( $this->getPath(), $file );
			chmod( $file, fileperms( $this->getPath() ) );
			$this->file    =& Phoenix_Framework_Flourish_Filesystem::hookFilenameMap( $file );
			$this->deleted =& Phoenix_Framework_Flourish_Filesystem::hookDeletedMap( $file );
			// Allow filesystem transactions
			if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::recordDuplicate( $this );
			}
		}
		 ...
		 *
		 * If multiple Phoenix_Framework_Flourish_File objects are created for a single file, they will
		 * reflect changes in each other including rename and delete actions.
		 *
		 * @throws Phoenix_Framework_Exception  When no file was specified, the file does not exist or the path specified is not a file
		 *
		...
		public function __construct( $file, $skip_checks = false ) {
			...
			if ( $this->deleted !== null && ! Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::updateDeletedMap( $file, null );
			}
		}
		...
		public function __toString() {
			try {
				return $this->getName();
			} catch ( Phoenix_Framework_Exception $e ) {
				return '';
			}
		}
		...
		public function __wakeup() {
			...
			$this->deleted =& Phoenix_Framework_Flourish_Filesystem::hookDeletedMap( $file );
			if ( $deleted !== null ) {
				Phoenix_Framework_Flourish_Filesystem::updateDeletedMap( $file, $deleted );
			}
		}
		 ...
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return \Phoenix_Framework_Flourish_File The file object, to allow for method chaining
		 */
		public function append( $data ) {
			...
			// Allow filesystem transactions
			if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::recordAppend( $this, $data );
			}
		 ...
		 *
		 * @throws Phoenix_Framework_Exception   When there are no remaining lines in the file
		 * @internal
		 *
		...
		public function current() {
			...
			if ( $this->file_handle === null ) {
				...
				throw new Phoenix_Framework_Exception( 'There are no remaining lines' );
			}
		...
		public function delete() {
			...
			unlink( $this->file );
			Phoenix_Framework_Flourish_Filesystem::updateDeletedMap( $this->file, debug_backtrace() );
			Phoenix_Framework_Flourish_Filesystem::updateFilenameMap( $this->file, '*DELETED at ' . time() . ' with token ' . uniqid( '', true ) . '* ' . $this->file );
			return null;
		}
		 ...
		 * back.
		 *
		 * @param  string|Phoenix_Framework_Flourish_Directory $new_directory The directory to duplicate the file into
		 *                                                                    if different than the current directory
		 * @param  boolean                                     $overwrite     If a new directory is specified, this
		...
		public function duplicate( $new_directory = null, $overwrite = null ) {
			...
			if ( ! is_object( $new_directory ) ) {
				$new_directory = new Phoenix_Framework_Flourish_Directory( $new_directory );
			}
			$new_filename          = $new_directory->getPath() . $this->getName();
			$check_dir_permissions = false;
			if ( file_exists( $new_filename ) ) {
				if ( ! $overwrite ) {
					$new_filename          = Phoenix_Framework_Flourish_Filesystem::makeUniqueName( $new_filename );
			...
			// Allow filesystem transactions
			if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::recordDuplicate( $file );
			}
		...
		public function getExtension() {
			return Phoenix_Framework_Flourish_Filesystem::getPathInfo( $this->file, 'extension' );
		}
		 ...
		 *
		 * @return Phoenix_Framework_Flourish_Timestamp  The timestamp of when the file was last modified
		 */
		public function getMTime() {
			$this->tossIfDeleted();

			return new Phoenix_Framework_Flourish_Timestamp( filemtime( $this->file ) );
		}
		...
		public function getName( $remove_extension = false ) {
			...
			return Phoenix_Framework_Flourish_Filesystem::getPathInfo( $this->file, $remove_extension ? 'filename' : 'basename' );
		}

		/**
		 * Gets the directory the file is located in
		 *
		 * @return Phoenix_Framework_Flourish_Directory  The directory containing the file
		 */
		public function getParent() {
			return new Phoenix_Framework_Flourish_Directory( Phoenix_Framework_Flourish_Filesystem::getPathInfo( $this->file, 'dirname' ) );
		}
		...
		public function getPath( $translate_to_web_path = false ) {
			if ( $translate_to_web_path ) {
				return Phoenix_Framework_Flourish_Filesystem::translateToWebPath( $this->file );
			}
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When the directory passed is not a directory or is not readable
		 *
		 * @param  Phoenix_Framework_Flourish_Directory|string $new_directory The directory to move this file into
		 * @param  boolean                                     $overwrite     If the current filename already exists in
		 *                                                                    the new directory, `TRUE` will cause the
		 *                                                                    file to be overwritten, `FALSE` will
		 *                                                                    cause the new filename to change
		 *
		 * @return Phoenix_Framework_Flourish_File  The file object, to allow for method chaining
		 */
		public function move( $new_directory, $overwrite ) {
			if ( ! $new_directory instanceof Phoenix_Framework_Flourish_Directory ) {
				$new_directory = new Phoenix_Framework_Flourish_Directory( $new_directory );
			}
		 ...
		 * @param  mixed   $filename Present the file as an attachment instead of just outputting type headers - if a
		 *                           string is passed, that will be used for the filename, if `TRUE` is passed, the
		 *                           current filename will be used
		 *
		 * @return Phoenix_Framework_Flourish_File  The file object, to allow for method chaining
		 */
		 ...
		 * @param  boolean $overwrite    If the new filename already exists, `TRUE` will cause the file to be
		 *                               overwritten, `FALSE` will cause the new filename to change
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return \Phoenix_Framework_Flourish_File The file object, to allow for method chaining
		 */
		public function rename( $new_filename, $overwrite ) {
			...
			$info = Phoenix_Framework_Flourish_Filesystem::getPathInfo( $new_filename );
			...
			$new_filename = Phoenix_Framework_Flourish_Directory::makeCanonical( realpath( $info[ 'dirname' ] ) ) . $info[ 'basename' ];
			...
			if ( file_exists( $new_filename ) ) {
				...
				if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
					Phoenix_Framework_Flourish_Filesystem::recordWrite( new Phoenix_Framework_Flourish_File( $new_filename ) );
				}
				// Windows requires that the existing file be deleted before being replaced
				unlink( $new_filename );
			} else {
				$new_dir = new Phoenix_Framework_Flourish_Directory( $info[ 'dirname' ] );
			...
			// Allow filesystem transactions
			if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::recordRename( $this->file, $new_filename );
			}
			Phoenix_Framework_Flourish_Filesystem::updateFilenameMap( $this->file, $new_filename );
		...
		/**
		 * Throws an Phoenix_Framework_Exception if the file has been deleted
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return void
		 */
		...
		public function write( $data ) {
			...
			// Allow filesystem transactions
			if ( Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
				Phoenix_Framework_Flourish_Filesystem::recordWrite( $this );
			}
