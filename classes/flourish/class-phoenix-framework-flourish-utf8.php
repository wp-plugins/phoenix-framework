<?php
	 ...
	 *
	 * @version    1.0.0b16
	 * @changes    1.0.0b16  Added code to ::clean() to use mbstring if available since recent versions of iconv and
	 *             `//IGNORE` now return `FALSE` for bad encodings [wb, 2012-09-21]
	 ...
	 * @changes    1.0.0b11  Updated the class to not using phpinfo() to determine the iconv implementation [wb,
	 *             2010-11-04]
	 * @changes    1.0.0b10  Fixed a bug with capitalizing a lowercase i resulting in a dotted upper-case I [wb,
	 *             2010-11-01]
	 * @changes    1.0.0b9   Updated class to use fCore::startErrorCapture() instead of `error_reporting()` [wb,
	 *             2010-08-09]
	 ...
	 * @changes    1.0.0b6   Fixed ::clean() to work with PHP installs that use an iconv library that doesn't support
	 *             //IGNORE [wb, 2010-03-02]
	 * @changes    1.0.0b5   Changed ::ucwords() to also uppercase words right after various punctuation [wb,
	 *             2009-09-18]
	 ...
	 */
	class Phoenix_Framework_Flourish_UTF8 {
		// The following constants allow for nice looking callbacks to static methods
		const ascii = 'Phoenix_Framework_Flourish_UTF8::ascii';
		const chr = 'Phoenix_Framework_Flourish_UTF8::chr';
		const clean = 'Phoenix_Framework_Flourish_UTF8::clean';
		const cmp = 'Phoenix_Framework_Flourish_UTF8::cmp';
		const explode = 'Phoenix_Framework_Flourish_UTF8::explode';
		const icmp = 'Phoenix_Framework_Flourish_UTF8::icmp';
		const inatcmp = 'Phoenix_Framework_Flourish_UTF8::inatcmp';
		const ipos = 'Phoenix_Framework_Flourish_UTF8::ipos';
		const ireplace = 'Phoenix_Framework_Flourish_UTF8::ireplace';
		const irpos = 'Phoenix_Framework_Flourish_UTF8::irpos';
		const istr = 'Phoenix_Framework_Flourish_UTF8::istr';
		const len = 'Phoenix_Framework_Flourish_UTF8::len';
		const lower = 'Phoenix_Framework_Flourish_UTF8::lower';
		const ltrim = 'Phoenix_Framework_Flourish_UTF8::ltrim';
		const natcmp = 'Phoenix_Framework_Flourish_UTF8::natcmp';
		const ord = 'Phoenix_Framework_Flourish_UTF8::ord';
		const pad = 'Phoenix_Framework_Flourish_UTF8::pad';
		const pos = 'Phoenix_Framework_Flourish_UTF8::pos';
		const replace = 'Phoenix_Framework_Flourish_UTF8::replace';
		const reset = 'Phoenix_Framework_Flourish_UTF8::reset';
		const rev = 'Phoenix_Framework_Flourish_UTF8::rev';
		const rpos = 'Phoenix_Framework_Flourish_UTF8::rpos';
		const rtrim = 'Phoenix_Framework_Flourish_UTF8::rtrim';
		const str = 'Phoenix_Framework_Flourish_UTF8::str';
		const sub = 'Phoenix_Framework_Flourish_UTF8::sub';
		const trim = 'Phoenix_Framework_Flourish_UTF8::trim';
		const ucfirst = 'Phoenix_Framework_Flourish_UTF8::ucfirst';
		const ucwords = 'Phoenix_Framework_Flourish_UTF8::ucwords';
		const upper = 'Phoenix_Framework_Flourish_UTF8::upper';
		const wordwrap = 'Phoenix_Framework_Flourish_UTF8::wordwrap';
		/**
		...
		/**
		 * All lowercase UTF-8 characters not properly handled by [http://php.net/mb_strtoupper mb_strtoupper()] mapped
		...
		/**
		 * All uppercase UTF-8 characters not properly handled by [http://php.net/mb_strtolower mb_strtolower()] mapped
		...
		static public function clean( $value ) {
			if ( ! is_array( $value ) ) {
				...
				Phoenix_Framework_Flourish_Core::startErrorCapture( E_NOTICE );
		...
		/**
		 * Compares strings, with the resulting order having latin characters that are based on ASCII letters placed
		...
		/**
		 * Converts an offset in characters to an offset in bytes to that we can use the built-in functions for some
		 * operations
		 *
		 ...
		 * @param  string $delimiter The string to explode on. If `NULL` or `''` this method will return one character
		 *                           per array index.
		 *
		...
		/**
		 * Compares strings in a case-insensitive manner, with the resulting order having characters that are based on
		 * ASCII letters placed after the relative ASCII characters
		 *
		 * Please note that this function sorts based on English language sorting
		 * rules only. Locale-sepcific sorting is done by
		 * [http://php.net/strcoll strcoll()], however there are technical
		...
		/**
		 * Compares strings using a natural order algorithm in a case-insensitive manner, with the resulting order
		 * having latin characters that are based on ASCII letters placed after the relative ASCII characters
		 *
		 * Please note that this function sorts based on English language sorting
		 * rules only. Locale-sepcific sorting is done by
		 * [http://php.net/strcoll strcoll()], however there are technical
		...
		/**
		 * Finds the first position (in characters) of the search value in the string - case is ignored when doing
		 * performing a match
		 *
		...
		/**
		 * Finds the last position (in characters) of the search value in the string - case is ignored when doing
		 * performing a match
		 *
		 * @param  string  $haystack The string to search in
		 * @param  string  $needle   The string to search for. This match will be done in a case-insensitive manner.
		 * @param  integer $offset   The character position to start searching from. A negative value will stop looking
		...
		/**
		 * Matches a string needle in the string haystack, returning a substring from the beginning of the needle to
		 ...
		 *
		 * @param  string  $haystack      The string to search in
		 * @param  string  $needle        The string to search for. This match will be done in a case-insensitive
		 *                                manner.
		 * @param  boolean $before_needle If a substring of the haystack before the needle should be returned instead
		 *                                of the substring from the needle to the end of the haystack
		 *
		...
		/**
		 * Compares strings using a natural order algorithm, with the resulting order having latin characters that are
		 ...
		 * @param  integer $start  The zero-based starting index to extract from. Negative values will start the
		 *                         extraction that many characters from the end of the string.
		 * @param  integer $length The length of the string to extract. If an empty value is provided, the remainder of
		 ...
		 *
		 * @return Phoenix_Framework_Flourish_UTF8
		 */
