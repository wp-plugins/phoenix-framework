<?php
	 ...
	 * @changes    1.0.0b24  Backwards Compatibility Break - moved ::detectOpcodeCache() to fLoader::hasOpcodeCache()
	 *             [wb, 2011-08-26]
	 * @changes    1.0.0b23  Backwards Compatibility Break - changed the email subject of error/exception emails to
	 *             include relevant file info, instead of the timestamp, for better email message threading [wb,
	 *             2011-06-20]
	 * @changes    1.0.0b22  Fixed a bug with dumping arrays containing integers [wb, 2011-05-26]
	 * @changes    1.0.0b21  Changed ::startErrorCapture() to allow "stacking" it via multiple calls, fixed a couple of
	 *             bugs with ::dump() mangling strings in the form `int(1)`, fixed mispelling of `occurred` [wb,
	 *             2011-05-09]
	 * @changes    1.0.0b20  Backwards Compatibility Break - Updated ::expose() to not wrap the data in HTML when
	 *             running via CLI, and instead just append a newline [wb, 2011-02-24]
	 ...
	 * @changes    1.0.0b17  Fixed a bug with ::backtrace() triggering notices when an argument is not UTF-8 [wb,
	 *             2010-08-17]
	 * @changes    1.0.0b16  Added the `$types` and `$regex` parameters to ::startErrorCapture() and the `$regex`
	 *             parameter to ::stopErrorCapture() [wb, 2010-08-09]
	 * @changes    1.0.0b15  Added ::startErrorCapture() and ::stopErrorCapture() [wb, 2010-07-05]
	 * @changes    1.0.0b14  Changed ::enableExceptionHandling() to only call Phoenix_Framework_Exception::printMessage() when the
	 *             destination is not `html` and no callback has been defined, added ::configureSMTP() to allow using
	 *             fSMTP for error and exception emails [wb, 2010-06-04]
	 * @changes    1.0.0b13  Added the `$backtrace` parameter to ::backtrace() [wb, 2010-03-05]
	 * @changes    1.0.0b12  Added ::getDebug() to check for the global debugging flag, added more specific BSD checks
	 *             to ::checkOS() [wb, 2010-03-02]
	 * @changes    1.0.0b11  Added ::detectOpcodeCache() [nt+wb, 2009-10-06]
	 * @changes    1.0.0b10  Fixed ::expose() to properly display when output includes non-UTF-8 binary data [wb,
	 *             2009-06-29]
	 * @changes    1.0.0b9   Added ::disableContext() to remove context info for exception/error handling, tweaked
	 *             output for exceptions/errors [wb, 2009-06-28]
	 * @changes    1.0.0b8   ::enableErrorHandling() and ::enableExceptionHandling() now accept multiple email
	 *             addresses, and a much wider range of emails [wb-imarc, 2009-06-01]
	 * @changes    1.0.0b7   ::backtrace() now properly replaces document root with {doc_root} on Windows [wb,
	 *             2009-05-02]
	 * @changes    1.0.0b6   Fixed a bug with getting the server name for error messages when running on the command
	 *             line [wb, 2009-03-11]
	 * @changes    1.0.0b5   Fixed a bug with checking the error/exception destination when a log file is specified
	 *             [wb, 2009-03-07]
	 * @changes    1.0.0b4   Backwards compatibility break - ::getOS() and ::getPHPVersion() removed, replaced with
	 *             ::checkOS() and ::checkVersion() [wb, 2009-02-16]
	 ...
	 */
	class Phoenix_Framework_Flourish_Core {
		// The following constants allow for nice looking callbacks to static methods
		const backtrace = 'Phoenix_Framework_Flourish_Core::backtrace';
		const call = 'Phoenix_Framework_Flourish_Core::call';
		const callback = 'Phoenix_Framework_Flourish_Core::callback';
		const checkOS = 'Phoenix_Framework_Flourish_Core::checkOS';
		const checkVersion = 'Phoenix_Framework_Flourish_Core::checkVersion';
		const configureSMTP = 'Phoenix_Framework_Flourish_Core::configureSMTP';
		const debug = 'Phoenix_Framework_Flourish_Core::debug';
		const disableContext = 'Phoenix_Framework_Flourish_Core::disableContext';
		const dump = 'Phoenix_Framework_Flourish_Core::dump';
		const enableDebugging = 'Phoenix_Framework_Flourish_Core::enableDebugging';
		const enableDynamicConstants = 'Phoenix_Framework_Flourish_Core::enableDynamicConstants';
		const enableErrorHandling = 'Phoenix_Framework_Flourish_Core::enableErrorHandling';
		const enableExceptionHandling = 'Phoenix_Framework_Flourish_Core::enableExceptionHandling';
		const expose = 'Phoenix_Framework_Flourish_Core::expose';
		const getDebug = 'Phoenix_Framework_Flourish_Core::getDebug';
		const handleError = 'Phoenix_Framework_Flourish_Core::handleError';
		const handleFatalError = 'Phoenix_Framework_Flourish_Core::handleFatalError';
		const handleException = 'Phoenix_Framework_Flourish_Core::handleException';
		const registerDebugCallback = 'Phoenix_Framework_Flourish_Core::registerDebugCallback';
		const reset = 'Phoenix_Framework_Flourish_Core::reset';
		const sendMessagesOnShutdown = 'Phoenix_Framework_Flourish_Core::sendMessagesOnShutdown';
		const startErrorCapture = 'Phoenix_Framework_Flourish_Core::startErrorCapture';
		const stopErrorCapture = 'Phoenix_Framework_Flourish_Core::stopErrorCapture';
		/**
		 ...
		 * @param  array   $backtrace    A backtrace from [http://php.net/backtrace `debug_backtrace()`] to format -
		 *                               this is not usually required or desired
		 *
		...
		/**
		 * Performs a [http://php.net/call_user_func call_user_func()], while translating PHP 5.2 static callback
		 * syntax for PHP 5.1 and 5.0
		 *
		 ...
		 * Phoenix_Framework_Flourish_Core::call('Class::method', TRUE, 0, 'test');
		 *
		 * // Passing multiple parameters in a parameters array
		 * Phoenix_Framework_Flourish_Core::call('Class::method', array(TRUE, 0, 'test'));
		 * }}}
		 *
		 ...
		 * Phoenix_Framework_Flourish_Core::call('Class::method', array(&$var1, &$var2));
		 * }}}
		 *
		...
		/**
		 * Translates a Class::method style static method callback to array style for compatibility with PHP 5.0 and
		 * 5.1 and built-in PHP functions
		 *
		...
		static public function checkOS( $os ) {
			...
			throw new Phoenix_Framework_Exception( 'Unable to determine the current OS' );
		}
		...
		/**
		 * Creates a string representation of any variable using predefined strings for booleans, `NULL` and empty
		 * strings
		 *
		...
		/**
		 * Turns on a feature where undefined constants are automatically created with the string value equivalent to
		 * the name
		 *
		...
		/**
		 * Turns on developer-friendly error handling that includes context information including a backtrace and
		 * superglobal dumps
		 *
		 ...
		 *
		 * @param  string  $destination          The destination for the errors and context information - an email
		 *                                       address, a file path or the string `'html'`
		 * @param  boolean $handle_fatal_errors  If true, a shutdown function will be registered to handle a fatal
		 *                                       error. Be aware, other shutdown functions could inadvertantly disable
		 *                                       this one or exit the process.
		 *
		 * @return void
		 */
		...
		/**
		 * Turns on developer-friendly uncaught exception handling that includes context information including a
		 ...
		 *
		 * @param  string   $destination  The destination for the exception and context information - an email address,
		 *                                a file path or the string `'html'`
		 * @param  callback $closing_code This callback will happen after the exception is handled and before page
		 *                                execution stops. Good for printing a footer. If no callback is provided and
		 *                                the exception extends Phoenix_Framework_Exception, Phoenix_Framework_Exception::printMessage() will be called.
		...
		static public function handleError( $error_number, $error_string, $error_file = null, $error_line = null, $error_context = null ) {
			...
			$backtrace    = preg_replace( '#: Phoenix_Framework_Flourish_Core::handleError\(.*?\)$#', '', $backtrace );
			...
			switch ( $error_number ) {
				case E_WARNING:
					$type = self::compose( 'Warning' );
					break;
				case E_NOTICE:
					$type = self::compose( 'Notice' );
					break;
				case E_USER_ERROR:
					$type = self::compose( 'User Error' );
					break;
				case E_USER_WARNING:
					$type = self::compose( 'User Warning' );
					break;
				case E_USER_NOTICE:
					$type = self::compose( 'User Notice' );
					break;
				case E_STRICT:
					$type = self::compose( 'Strict' );
					break;
				case E_RECOVERABLE_ERROR:
					$type = self::compose( 'Recoverable Error' );
					break;
				case E_DEPRECATED:
					$type = self::compose( 'Deprecated' );
					break;
				case E_USER_DEPRECATED:
					$type = self::compose( 'User Deprecated' );
					break;
				case E_CORE_ERROR:
					$type = self::compose( 'Fatal Error' );
					break;
				case E_ERROR:
					$type = self::compose( 'Fatal Error' );
					break;
			...
			if ( $capturing ) {
				...
				if ( self::$captured_errors_previous_handler[ self::$captured_error_level ] != array(
						'Phoenix_Framework_Flourish_Core',
						'handleError'
					)
				) {
		...
		/**
		 * Handles an uncaught exception, creating the necessary context information, sending it to the specified
		 * destination and finally executing the closing callback
		 *
		 * @internal
		 *
		...
		static public function handleException( $exception ) {
			...
			if ( $exception instanceof Phoenix_Framework_Exception ) {
			...
			if ( self::$exception_handler_callback === null ) {
				if ( self::$exception_destination != 'html' && $exception instanceof Phoenix_Framework_Exception ) {
		...
		/**
		 * Registers a callback to handle debug messages instead of the default action of calling ::expose() on the
		 * message
		 *
		 ...
		 *
		 * @param  integer $types The error types to capture - this should be as specific as possible - defaults to all
		 *                        (E_ALL | E_STRICT)
		 ...
		 *
		 * @return Phoenix_Framework_Flourish_Core
		 */
