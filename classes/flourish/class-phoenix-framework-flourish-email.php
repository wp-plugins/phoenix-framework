<?php
	 ...
	 * @changes    1.0.0b29  Changed ::combineNameEmail() to be a static method and to be exposed publicly for use by
	 *             other classes [wb, 2011-07-26]
	 * @changes    1.0.0b28  Fixed ::addAttachment() and ::addRelatedFile() to properly handle duplicate filenames [wb,
	 *             2011-05-17]
	 ...
	 * @changes    1.0.0b24  Backwards Compatibility Break - the `$contents` parameter of ::addAttachment() is now
	 *             first instead of third, ::addAttachment() will now accept Phoenix_Framework_Flourish_File objects for the `$contents`
	 *             parameter, added ::addRelatedFile() [wb, 2010-12-01]
	 * @changes    1.0.0b23  Fixed a bug on Windows where emails starting with a `.` would have the `.` removed [wb,
	 *             2010-09-11]
	 * @changes    1.0.0b22  Revamped the FQDN code and added ::getFQDN() [wb, 2010-09-07]
	 * @changes    1.0.0b21  Added a check to prevent permissions warnings when getting the FQDN on Windows machines
	 *             [wb, 2010-09-02]
	 * @changes    1.0.0b20  Fixed ::send() to only remove the name of a recipient when dealing with the `mail()`
	 *             function on Windows and to leave it when using fSMTP [wb, 2010-06-22]
	 * @changes    1.0.0b19  Changed ::send() to return the message id for the email, fixed the email regexes to
	 *             require [] around IPs [wb, 2010-05-05]
	 ...
	 * @changes    1.0.0b15  Added the `$unindent_expand_constants` parameter to ::setBody(), added ::loadBody() and
	 *             ::loadHTMLBody(), fixed HTML emails with attachments [wb, 2010-03-14]
	 * @changes    1.0.0b14  Changed ::send() to not double `.`s at the beginning of lines on Windows since it seemed
	 *             to break things rather than fix them [wb, 2010-03-05]
	 * @changes    1.0.0b13  Fixed the class to work when safe mode is turned on [wb, 2009-10-23]
	 * @changes    1.0.0b12  Removed duplicate MIME-Version headers that were being included in S/MIME encrypted emails
	 *             [wb, 2009-10-05]
	 * @changes    1.0.0b11  Updated to use the new Phoenix_Framework_Exception API [wb, 2009-09-17]
	 ...
	 * @changes    1.0.0b6   Fixed a bug where <> quoted email addresses in validation messages were not showing [wb,
	 *             2009-03-27]
	 * @changes    1.0.0b5   Updated for new Phoenix_Framework_Flourish_Core API [wb, 2009-02-16]
	 * @changes    1.0.0b4   The recipient error message in ::validate() no longer contains a typo [wb, 2009-02-09]
	 * @changes    1.0.0b3   Fixed a bug with missing content in the Phoenix_Framework_Exception thrown by ::validate() [wb, 2009-01-14]
	 ...
	 */
	class Phoenix_Framework_Email {
		// The following constants allow for nice looking callbacks to static methods
		const combineNameEmail = 'Phoenix_Framework_Email::combineNameEmail';
		const fixQmail = 'Phoenix_Framework_Email::fixQmail';
		const getFQDN = 'Phoenix_Framework_Email::getFQDN';
		const reset = 'Phoenix_Framework_Email::reset';
		const unindentExpand = 'Phoenix_Framework_Email::unindentExpand';
		/**
		 ...
		 * @param  integer $first_line_prefix_length The length of any prefix applied to the first line of the encoded
		 *                                           word - this allows properly accounting for a header name
		 *
		...
		/**
		 * Returns `TRUE` for non-empty strings, numbers, objects, empty numbers and string-like numbers (such as `0`,
		 * `0.0`, `'0'`)
		 *
		...
		/**
		 * Initializes Phoenix_Framework_Email for creating message ids
		 *
		 * @return Phoenix_Framework_Email
		 */
		public function __construct() {
			$this->message_id = '<' . Phoenix_Framework_Flourish_Cryptography::randomString( 10, 'hexadecimal' ) . '.' . time() . '@' . self::getFQDN() . '>';
		}
		 ...
		 *
		 * @param  string|Phoenix_Framework_Flourish_File $contents  The contents of the file
		 * @param  string       $filename  The name to give the attachement - optional if `$contents` is an Phoenix_Framework_Flourish_File
		 *                                 object
		 * @param  string       $mime_type The mime type of the file - this allows overriding the mime type of the file
		 *                                 if incorrectly detected
		 *
		 * @return Phoenix_Framework_Email  The email object, to allow for method chaining
		 */
		...
		public function addCustomHeader( $name, $value = null ) {
			...
			$lower_name                          = Phoenix_Framework_Flourish_UTF8::lower( $name );
		...
		private function createSMIMEBody( $to, $subject, $headers, $body ) {
			...
			$plaintext_file  = tempnam( '', '__Phoenix_Framework_Email_' );
			$ciphertext_file = tempnam( '', '__Phoenix_Framework_Email_' );
			$headers_array   = array(
				'To'      => $to,
				'Subject' => $subject
			);
		 ...
		 *
		 * @param  string $recipients_smime_cert_file The file path to the PEM-encoded S/MIME certificate for the
		 *                                            recipient
		 *
		 * @return Phoenix_Framework_Email The email object, to allow for method chaining
		 * @throws Phoenix_Framework_Exception
		...
		/**
		 * Extracts the filename and mime-type from an Phoenix_Framework_Flourish_File object
		 *
		 * @param  string|Phoenix_Framework_Flourish_File &$contents  The file to extrapolate the info from
		...
		private function extrapolateFileInfo( &$contents, &$filename, &$mime_type ) {
			if ( $contents instanceof Phoenix_Framework_Flourish_File ) {
				if ( $filename === null ) {
				...
				if ( $mime_type === null ) {
					$mime_type = Phoenix_Framework_Flourish_File::determineMimeType( $filename, $contents );
				}
			}
		}
		...
		private function generateNewFilename( $filename ) {
			$filename_info = Phoenix_Framework_Flourish_Filesystem::getPathInfo( $filename );
		 ...
		 *
		 * @param  string|Phoenix_Framework_Flourish_File $file         The plaintext version of the email body
		 * @param  array        $replacements The method will search the contents of the file for each key and replace
		 *                                    it with the corresponding value
		 *
		 * @return Phoenix_Framework_Email  The email object, to allow for method chaining
		 */
		public function loadBody( $file, $replacements = array() ) {
			if ( ! $file instanceof Phoenix_Framework_Flourish_File ) {
				$file = new Phoenix_Framework_Flourish_File( $file );
			}
			$plaintext = $file->read();
			if ( $replacements ) {
		...
		public function send( $connection = null ) {
			...
			if ( $connection === null && Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) ) {
				$vars = array(
					'bcc_emails',
					'bounce_to_email',
					'cc_emails',
			...
			$parameters = null;
			if ( ! Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) && $this->bounce_to_email ) {
			...
			} elseif ( Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) && $this->bounce_to_email ) {
		 ...
		 * @param  boolean $unindent_expand_constants If this is `TRUE`, the body will be unindented as much as
		 *                                            possible and {CONSTANT_NAME} will be replaced with the value of
		 *                                            the constant
		 *
		 * @return Phoenix_Framework_Email  The email object, to allow for method chaining
		 */
		...
		public function setBounceToEmail( $email ) {
			if ( ini_get( 'safe_mode' ) && ! Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) ) {
				throw new Phoenix_Framework_Exception( 'It is not possible to set a Bounce-To Email address when safe mode is enabled on a non-Windows server' );
			}
			if ( ! $email ) {
				return;
			}
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When part of the email is missing or formatted incorrectly
		 *
		 * @return void
		 */
