<?php
	...
	/**
	 * Handles filesystem-level tasks including filesystem transactions and the reference map to keep all Phoenix_Framework_Flourish_File and
	 * Phoenix_Framework_Flourish_Directory objects in sync
	 *
	 ...
	 *
	 * @version    1.0.0b16
	 * @changes    1.0.0b16  Added a call to clearstatcache() to ::rollback() to prevent incorrect data from being
	 *             returned by Phoenix_Framework_Flourish_File::getMTime() and Phoenix_Framework_Flourish_File::getSize() [wb, 2010-11-27]
	 ...
	 * @changes    1.0.0b13  Changed the way files/directories deleted in a filesystem transaction are handled,
	 *             including improvements to the exception that is thrown [wb+wb-imarc, 2010-03-05]
	 * @changes    1.0.0b12  Updated ::convertToBytes() to properly handle integers without a suffix and sizes with
	 *             fractions [al+wb, 2009-11-14]
	 * @changes    1.0.0b11  Corrected the API documentation for ::getPathInfo() [wb, 2009-09-09]
	 * @changes    1.0.0b10  Updated ::updateExceptionMap() to not contain the Exception class parameter hint, allowing
	 *             NULL to be passed [wb, 2009-08-20]
	 * @changes    1.0.0b9   Added some performance tweaks to ::createObject() [wb, 2009-08-06]
	 * @changes    1.0.0b8   Changed ::formatFilesize() to not use decimal places for bytes, add a space before and
	 *             drop the `B` in suffixes [wb, 2009-07-12]
	 ...
	 * @changes    1.0.0b5   Changed ::formatFilesize() to use proper uppercase letters instead of lowercase [wb,
	 *             2009-06-02]
	 ...
	 * @changes    1.0.0b2   Fixed a bug where the filepath and exception maps weren't being updated after a rollback
	 *             [wb, 2008-12-11]
	 * @changes    1.0.0b    The initial implementation [wb, 2008-03-24]
	 */
	class Phoenix_Framework_Flourish_Filesystem {
		// The following constants allow for nice looking callbacks to static methods
		const addWebPathTranslation = 'Phoenix_Framework_Flourish_Filesystem::addWebPathTranslation';
		const begin = 'Phoenix_Framework_Flourish_Filesystem::begin';
		const commit = 'Phoenix_Framework_Flourish_Filesystem::commit';
		const convertToBytes = 'Phoenix_Framework_Flourish_Filesystem::convertToBytes';
		const createObject = 'Phoenix_Framework_Flourish_Filesystem::createObject';
		const formatFilesize = 'Phoenix_Framework_Flourish_Filesystem::formatFilesize';
		const getPathInfo = 'Phoenix_Framework_Flourish_Filesystem::getPathInfo';
		const hookDeletedMap = 'Phoenix_Framework_Flourish_Filesystem::hookDeletedMap';
		const hookFilenameMap = 'Phoenix_Framework_Flourish_Filesystem::hookFilenameMap';
		const isInsideTransaction = 'Phoenix_Framework_Flourish_Filesystem::isInsideTransaction';
		const makeUniqueName = 'Phoenix_Framework_Flourish_Filesystem::makeUniqueName';
		const recordAppend = 'Phoenix_Framework_Flourish_Filesystem::recordAppend';
		const recordCreate = 'Phoenix_Framework_Flourish_Filesystem::recordCreate';
		const recordDelete = 'Phoenix_Framework_Flourish_Filesystem::recordDelete';
		const recordDuplicate = 'Phoenix_Framework_Flourish_Filesystem::recordDuplicate';
		const recordRename = 'Phoenix_Framework_Flourish_Filesystem::recordRename';
		const recordWrite = 'Phoenix_Framework_Flourish_Filesystem::recordWrite';
		const reset = 'Phoenix_Framework_Flourish_Filesystem::reset';
		const rollback = 'Phoenix_Framework_Flourish_Filesystem::rollback';
		const translateToWebPath = 'Phoenix_Framework_Flourish_Filesystem::translateToWebPath';
		const updateDeletedMap = 'Phoenix_Framework_Flourish_Filesystem::updateDeletedMap';
		const updateFilenameMap = 'Phoenix_Framework_Flourish_Filesystem::updateFilenameMap';
		const updateFilenameMapForDirectory = 'Phoenix_Framework_Flourish_Filesystem::updateFilenameMapForDirectory';
		/**
		...
		static public function addWebPathTranslation( $search_path, $replace_path ) {
			...
			$search_path                                 = str_replace( array(
					'/',
					'\\'
				), DIRECTORY_SEPARATOR, $search_path );
			$replace_path                                = str_replace( array(
					'/',
					'\\'
				), DIRECTORY_SEPARATOR, $replace_path );
		...
		/**
		 * Takes a filesystem path and creates either an Phoenix_Framework_Flourish_Directory, Phoenix_Framework_Flourish_File or Phoenix_Framework_Flourish_Image object from it
		 *
		 * @throws Exception  When no path was specified or the path specified does not exist
		 *
		 * @param  string $path The path to the filesystem object
		 *
		 * @return Phoenix_Framework_Flourish_Directory|Phoenix_Framework_Flourish_File|Phoenix_Framework_Flourish_Image
		 */
		static public function createObject( $path ) {
			...
			if ( is_dir( $path ) ) {
				return new Phoenix_Framework_Flourish_Directory( $path, true );
			}
			if ( Phoenix_Framework_Flourish_Image::isImageCompatible( $path ) ) {
				return new Phoenix_Framework_Flourish_Image( $path, true );
			}

			return new Phoenix_Framework_Flourish_File( $path, true );
		}
		 ...
		 * @param  string $element The piece of information to return: `'dirname'`, `'basename'`, `'extension'`, or
		 *                         `'filename'`
		 * @throws Exception
		...
		/**
		 * Changes a filename to be safe for URLs by making it all lower case and changing everything but letters,
		 * numers, - and . to _
		 *
		 ...
		 *
		 * @param  Phoenix_Framework_Flourish_File  $file The file that is being written to
		 ...
		 *
		 * @param  Phoenix_Framework_Flourish_File|Phoenix_Framework_Flourish_Directory $object The filesystem object to delete
		 *
		 * @return array
		 */
		static public function recordDelete( $object ) {
			...
			return self::$commit_operations;
		}

		/**
		 * Keeps a record of duplicated files so they can be cleaned up in case of a rollback
		 *
		 * @param  Phoenix_Framework_Flourish_File $file The duplicate file to get rid of on rollback
		 *
		 * @return void
		 */
		...
		static public function rollback() {
			...
			foreach ( self::$rollback_operations as $operation ) {
				switch ( $operation[ 'action' ] ) {
					...
					case 'delete':
						...
						unlink( $operation[ 'filename' ] );
						Phoenix_Framework_Flourish_Filesystem::updateFilenameMap( $operation[ 'filename' ], '*DELETED at ' . time() . ' with token ' . uniqid( '', true ) . '* ' . $operation[ 'filename' ] );
						break;
					case 'write':
						...
						break;
					case 'rename':
						Phoenix_Framework_Flourish_Filesystem::updateFilenameMap( $operation[ 'new_name' ], $operation[ 'old_name' ] );
			...
			foreach ( self::$commit_operations as $operation ) {
				if ( isset( $operation[ 'object' ] ) ) {
					...
					Phoenix_Framework_Flourish_Filesystem::updateFilenameMap( $operation[ 'object' ]->getPath(), preg_replace( '#*DELETED at \d+ with token [\w.]+* #', '', $operation[ 'filename' ] ) );
				}
			}
			self::$commit_operations   = null;
			self::$rollback_operations = null;
		 ...
		 *
		 * @return Phoenix_Framework_Flourish_Filesystem
		 */
