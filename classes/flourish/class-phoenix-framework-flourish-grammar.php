<?php
	 ...
	 *
	 * @version    1.0.0b15
	 * @changes    1.0.0b15  Added length checking to ensure blank strings are not being passed to various methods [wb,
	 *             2011-06-20]
	 * @changes    1.0.0b14  Fixed a bug in singularization that would affect words containing the substring `mice` or
	 *             `lice` [wb, 2011-02-24]
	 * @changes    1.0.0b13  Fixed the pluralization of video [wb, 2010-08-10]
	 * @changes    1.0.0b12  Updated ::singularize() and ::pluralize() to be able to handle underscore_CamelCase [wb,
	 *             2010-08-06]
	 ...
	 * @changes    1.0.0b5   Fixed ::reset() to properly reset the singularization and pluralization rules [wb,
	 *             2009-10-28]
	 * @changes    1.0.0b4   Added caching for various methods - provided significant performance boost to ORM [wb,
	 *             2009-06-15]
	 * @changes    1.0.0b3   Changed replacement values in preg_replace() calls to be properly escaped [wb, 2009-06-11]
	 * @changes    1.0.0b2   Fixed a bug where some words would lose capitalization with ::pluralize() and
	 *             ::singularize() [wb, 2009-01-25]
	 * @changes    1.0.0b    The initial implementation [wb, 2007-09-25]
	 */
	class Phoenix_Framework_Flourish_Grammar {
		// The following constants allow for nice looking callbacks to static methods
		const addCamelUnderscoreRule = 'Phoenix_Framework_Flourish_Grammar::addCamelUnderscoreRule';
		const addHumanizeRule = 'Phoenix_Framework_Flourish_Grammar::addHumanizeRule';
		const addSingularPluralRule = 'Phoenix_Framework_Flourish_Grammar::addSingularPluralRule';
		const camelize = 'Phoenix_Framework_Flourish_Grammar::camelize';
		const humanize = 'Phoenix_Framework_Flourish_Grammar::humanize';
		const inflectOnQuantity = 'Phoenix_Framework_Flourish_Grammar::inflectOnQuantity';
		const joinArray = 'Phoenix_Framework_Flourish_Grammar::joinArray';
		const pluralize = 'Phoenix_Framework_Flourish_Grammar::pluralize';
		const registerJoinArrayCallback = 'Phoenix_Framework_Flourish_Grammar::registerJoinArrayCallback';
		const reset = 'Phoenix_Framework_Flourish_Grammar::reset';
		const singularize = 'Phoenix_Framework_Flourish_Grammar::singularize';
		const stem = 'Phoenix_Framework_Flourish_Grammar::stem';
		const underscorize = 'Phoenix_Framework_Flourish_Grammar::underscorize';
		/**
		 ...
		 * @param  string  $plural_form                 The string to be returned for when `$quantity != 1`, use `%d`
		 ...
		 * @param  boolean $return_error  If this is `TRUE` and the noun can't be pluralized, `FALSE` will be returned
		 *                                instead
		 *
		 * @throws Exception
		 ...
		 *
		 * @param  callback $callback The function to replace ::joinArray() with - should accept the same parameters
		...
		static public function stem( $word ) {
			...
			$word       = Phoenix_Framework_Flourish_UTF8::ascii( $word );
		 ...
		 *
		 * @return Phoenix_Framework_Flourish_Grammar
		 */
