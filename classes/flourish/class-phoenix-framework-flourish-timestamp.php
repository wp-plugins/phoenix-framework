<?php
	 ...
	 * @changes    1.0.0b12  Fixed a bug with the constructor not properly handling unix timestamps that are negative
	 *             integers [wb, 2011-06-02]
	 * @changes    1.0.0b11  Changed the `$timestamp` and `$timezone` attributes to be protected [wb, 2011-03-20]
	 * @changes    1.0.0b10  Fixed a bug in ::__construct() with specifying a timezone other than the default for a
	 *             relative time string such as "now" or "+2 hours" [wb, 2010-07-05]
	 ...
	 * @changes    1.0.0b6   Added ::registerUnformatCallback() and ::callUnformatCallback() to allow for localization
	 *             of date/time parsing [wb, 2009-06-01]
	 * @changes    1.0.0b5   Backwards compatibility break - Removed ::getSecondsDifference() and ::getSeconds(), added
	 *             ::eq(), ::gt(), ::gte(), ::lt(), ::lte() [wb, 2009-03-05]
	 * @changes    1.0.0b4   Updated for new fCore API [wb, 2009-02-16]
	 * @changes    1.0.0b3   Removed a useless double check of the strtotime() return value in ::__construct() [wb,
	 *             2009-01-21]
	 * @changes    1.0.0b2   Added support for CURRENT_TIMESTAMP, CURRENT_DATE and CURRENT_TIME SQL keywords [wb,
	 *             2009-01-11]
	 * @changes    1.0.0b    The initial implementation [wb, 2008-02-12]
	 */
	class Phoenix_Framework_Flourish_Timestamp {
		// The following constants allow for nice looking callbacks to static methods
		const callFormatCallback = 'Phoenix_Framework_Flourish_Timestamp::callFormatCallback';
		const callUnformatCallback = 'Phoenix_Framework_Flourish_Timestamp::callUnformatCallback';
		const combine = 'Phoenix_Framework_Flourish_Timestamp::combine';
		const defineFormat = 'Phoenix_Framework_Flourish_Timestamp::defineFormat';
		const fixISOWeek = 'Phoenix_Framework_Flourish_Timestamp::fixISOWeek';
		const getDefaultTimezone = 'Phoenix_Framework_Flourish_Timestamp::getDefaultTimezone';
		const isValidTimezone = 'Phoenix_Framework_Flourish_Timestamp::isValidTimezone';
		const registerFormatCallback = 'Phoenix_Framework_Flourish_Timestamp::registerFormatCallback';
		const registerUnformatCallback = 'Phoenix_Framework_Flourish_Timestamp::registerUnformatCallback';
		const reset = 'Phoenix_Framework_Flourish_Timestamp::reset';
		const setDefaultTimezone = 'Phoenix_Framework_Flourish_Timestamp::setDefaultTimezone';
		const translateFormat = 'Phoenix_Framework_Flourish_Timestamp::translateFormat';
		/**
		...
		static private function checkPHPVersion() {
			if ( ! Phoenix_Framework_Flourish_Core::checkVersion( '5.1' ) ) {
				throw new Exception(
		...
		/**
		 * Creates a reusable format for formatting fDate, fTime, and Phoenix_Framework_Flourish_Timestamp objects
		 *
		...
		/**
		 * Provides a consistent interface to getting the default timezone. Wraps the
		 * [http://php.net/date_default_timezone_get date_default_timezone_get()] function.
		 *
		...
		/**
		 * Allows setting a callback to translate or modify any return values from ::format(), fDate::format() and
		 * fTime::format()
		 *
		 * @param  callback $callback The callback to pass all formatted dates/times/timestamps through. Should accept
		 *                            a single string and return a single string.
		 *
		 * @return void
		 */
		...
		/**
		 * Allows setting a callback to parse any date strings passed into ::__construct(), fDate::__construct() and
		 * fTime::__construct()
		 *
		 * @param  callback $callback The callback to pass all date/time/timestamp strings through. Should accept a
		 *                            single string and return a single string that is parsable by
		 *                            [http://php.net/strtotime `strtotime()`].
		 *
		 * @return void
		 */
		...
		/**
		 * Provides a consistent interface to setting the default timezone. Wraps the
		 * [http://php.net/date_default_timezone_set date_default_timezone_set()] function.
		 *
		...
		/**
		 * Takes a format name set via ::defineFormat() and returns the [http://php.net/date date()] function
		 * formatting string
		 *
		 * @internal
		 *
		 ...
		 *
		 * @return string  The formatting string. If no matching format was found, this will be the same as the
		 *                 `$format` parameter.
		 */
		 ...
		 *
		 * @throws Exception  When `$datetime` is not a valid date/time, date or time value
		 *
		 * @param  Phoenix_Framework_Flourish_Timestamp|object|string|integer $datetime The date/time to represent, `NULL` is interpreted as now
		 * @param  string                           $timezone The timezone for the date/time. This causes the date/time
		 *                                                    to be interpretted as being in the specified timezone. If
		 *                                                    not specified, will default to timezone set by
		 *                                                    ::setDefaultTimezone().
		 *
		 * @return Phoenix_Framework_Flourish_Timestamp
		 */
		public function __construct( $datetime = null, $timezone = null ) {
			...
			} elseif ( $datetime instanceof Phoenix_Framework_Flourish_Timestamp ) {
			...
			} elseif ( is_string( $datetime ) && in_array( strtoupper( $datetime ), array(
						'CURRENT_TIMESTAMP',
						'CURRENT_TIME'
					) )
			) {
		 ...
		 *
		 * @throws Exception  When `$adjustment` is not a valid relative date/time measurement or timezone
		 *
		 * @param  string $adjustment The adjustment to make - may be a relative adjustment or a different timezone
		 *
		 * @return Phoenix_Framework_Flourish_Timestamp  The adjusted date/time
		 */
		public function adjust( $adjustment ) {
			...
			return new Phoenix_Framework_Flourish_Timestamp( $timestamp, $timezone );
		}

		/**
		 * If this timestamp is equal to the timestamp passed
		 *
		 * @param  Phoenix_Framework_Flourish_Timestamp|object|string|integer $other_timestamp The timestamp to compare with, `NULL` is
		 *                                                           interpreted as today
		 *
		 * @return boolean  If this timestamp is equal to the one passed
		 */
		public function eq( $other_timestamp = null ) {
			$other_timestamp = new Phoenix_Framework_Flourish_Timestamp( $other_timestamp );
		 ...
		 *
		 * @param  string $format The [http://php.net/date date()] function compatible formatting string, or a format
		 *                        name from ::defineFormat()
		 *
		 ...
		 *
		 * @param  Phoenix_Framework_Flourish_Timestamp|object|string|integer  $other_timestamp The timestamp to create the difference with,
		 *                                                            `NULL` is interpreted as now
		 * @param  boolean                           $simple          When `TRUE`, the returned value will only include
		 *                                                            the difference in the two timestamps, but not
		 *                                                            `from now`, `ago`, `after` or `before`
		...
		public function getFuzzyDifference( $other_timestamp = null, $simple = false ) {
			...
			foreach ( $break_points as $break_point => $unit_info ) {
				...
				$units     = Phoenix_Framework_Flourish_Grammar::inflectOnQuantity( $unit_diff, $unit_info[ 1 ], $unit_info[ 2 ] );
				break;
			}
			if ( $simple ) {
		...
		/**
		 * Modifies the current timestamp, creating a new Phoenix_Framework_Flourish_Timestamp object
		 *
		 ...
		 *
		 * @param  string $format   The current timestamp will be formatted with this string, and the output used to
		 *                          create a new object. The format should **not** include the timezone (character
		 *                          `e`).
		 * @param  string $timezone The timezone for the new object if different from the current timezone
		 *
		 * @return Phoenix_Framework_Flourish_Timestamp  The new timestamp
		 */
		public function modify( $format, $timezone = null ) {
			$timezone = ( $timezone !== null ) ? $timezone : $this->timezone;

			return new Phoenix_Framework_Flourish_Timestamp( $this->format( $format ), $timezone );
		}
	}
