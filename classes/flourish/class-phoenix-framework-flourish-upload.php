<?php
	 ...
	 * @changes    1.0.0b13  Changed the class to throw Exception objects instead of Exception objects when
	 *             the form is improperly configured - this is to prevent error logs when bad requests are sent by
	 *             scanners/hackers [wb, 2011-08-24]
	 ...
	 * @changes    1.0.0b10  BackwardsCompatibilityBreak - renamed ::setMaxFilesize() to ::setMaxSize() to be
	 *             consistent with Phoenix_Framework_Flourish_File::getSize() [wb, 2010-05-30]
	 * @changes    1.0.0b9   BackwardsCompatibilityBreak - the class no longer accepts uploaded files that start with a
	 *             `.` unless ::allowDotFiles() is called - added ::setOptional() [wb, 2010-05-30]
	 * @changes    1.0.0b8   BackwardsCompatibilityBreak - ::validate() no longer returns the `$_FILES` array for the
	 *             file being validated - added `$return_message` parameter to ::validate(), fixed a bug with detection
	 *             of mime type for text files [wb, 2010-05-26]
	 * @changes    1.0.0b7   Added ::filter() to allow for ignoring array file upload field entries that did not have a
	 *             file uploaded [wb, 2009-10-06]
	 * @changes    1.0.0b6   Updated ::move() to use the new Phoenix_Framework_Flourish_Filesystem::createObject()
	 *             method [wb, 2009-01-21]
	 * @changes    1.0.0b5   Removed some unnecessary error suppression operators from ::move() [wb, 2009-01-05]
	 * @changes    1.0.0b4   Updated ::validate() so it properly handles upload max filesize specified in
	 *             human-readable notation [wb, 2009-01-05]
	 ...
	 */
	class Phoenix_Framework_Flourish_Upload {
		// The following constants allow for nice looking callbacks to static methods
		const check = 'Phoenix_Framework_Flourish_Upload::check';
		const count = 'Phoenix_Framework_Flourish_Upload::count';
		const filter = 'Phoenix_Framework_Flourish_Upload::filter';

		/**
		 * Checks to see if the field specified is a valid file upload field
		 *
		 * @throws Exception  If `$throw_exception` is `TRUE` and the request was not a POST or the content type is not
		 *                    multipart/form-data
		 *
		 ...
		 *
		 * @throws Exception  If the form is not properly configured for file uploads
		 *
		 ...
		 * @param  mixed  $index If the field is an array file upload field, use this to specify which array index to
		 *                       return
		 *
		 * @throws Exception
		 ...
		 *
		 * @throws Exception  When the form is not setup for file uploads, the `$directory` is somehow invalid or
		 *                    ::validate() thows an exception
		 *
		 * @param  string|Phoenix_Framework_Flourish_Directory $directory The directory to upload the file to
		 * @param  string                                      $field     The file upload field to get the file from
		 * @param  mixed                                       $index     If the field was an array file upload field,
		 *                                                                upload the file corresponding to this index
		 *
		 * @return Phoenix_Framework_Flourish_File|NULL  An Phoenix_Framework_Flourish_File (or fImage) object, or
		...
		public function move( $directory, $field, $index = null ) {
			...
			$file_name = Phoenix_Framework_Flourish_Filesystem::makeURLSafe( $file_array[ 'name' ] );
			$file_name = $directory->getPath() . $file_name;
			if ( ! $this->enable_overwrite ) {
				$file_name = Phoenix_Framework_Flourish_Filesystem::makeUniqueName( $file_name );
			}
			...
			if ( ! chmod( $file_name, 0644 ) ) {
				throw new Exception( 'Unable to change permissions on the uploaded file' );
			}

			return Phoenix_Framework_Flourish_Filesystem::createObject( $file_name );
		}
		 ...
		 * Sets the allowable dimensions for an uploaded image
		 *
		 * @param  integer $width                  The minimum ratio width
		 * @param  integer $height                 The minimum ratio height
		...
		public function setMaxSize( $size ) {
			...
			$ini_max_size = ( ! is_numeric( $ini_max_size ) ) ? Phoenix_Framework_Flourish_Filesystem::convertToBytes( $ini_max_size ) : $ini_max_size;
			$size         = Phoenix_Framework_Flourish_Filesystem::convertToBytes( $size );
		...
		/**
		 * Validates the uploaded file, ensuring a file was actually uploaded and that is matched the restrictions put
		 * in place
		 *
		 * @throws Exception  When the form is not configured for file uploads, no file is uploaded or the uploaded
		 *                    file violates the options set for this object
		 *
		 * @param  string   $field          The field the file was uploaded through
		 * @param  mixed    $index          If the field was an array of file uploads, this specifies which one to
		 *                                  validate
		 * @param  boolean  $return_message If any validation error should be returned as a string instead of being
		 *                                  thrown as an Exception
		 * @param  string  |$field
		 * @param  boolean |$return_message
		 *
		 * @return NULL|string  If `$return_message` is not `TRUE` or if no error occurs, `NULL`, otherwise a string
		 *                      error message
		 */
		...
		private function validateField( $file_array ) {
			...
			if ( $file_array[ 'error' ] == UPLOAD_ERR_FORM_SIZE || $file_array[ 'error' ] == UPLOAD_ERR_INI_SIZE ) {
				...
				$max_size = ( ! is_numeric( $max_size ) ) ? Phoenix_Framework_Flourish_Filesystem::convertToBytes( $max_size ) : $max_size;

				return self::compose(
					'The file uploaded is over the limit of %s',
					Phoenix_Framework_Flourish_Filesystem::formatFilesize( $max_size )
				);
			}
			if ( $this->max_size && $file_array[ 'size' ] > $this->max_size ) {
				return self::compose(
					'The file uploaded is over the limit of %s',
					Phoenix_Framework_Flourish_Filesystem::formatFilesize( $this->max_size )
				);
			}
			...
			if ( ! empty( $this->mime_types ) && file_exists( $file_array[ 'tmp_name' ] ) ) {
				...
				if ( ! in_array( Phoenix_Framework_Flourish_File::determineMimeType( $file_array[ 'name' ], $contents ), $this->mime_types ) ) {
					return self::compose( $this->mime_type_message );
				}
			}
			if ( ! $this->allow_php ) {
				$file_info = Phoenix_Framework_Flourish_Filesystem::getPathInfo( $file_array[ 'name' ] );
			...
			if ( $this->image_dimensions && file_exists( $file_array[ 'tmp_name' ] ) ) {
				if ( Phoenix_Framework_Flourish_Image::isImageCompatible( $file_array[ 'tmp_name' ] ) ) {
