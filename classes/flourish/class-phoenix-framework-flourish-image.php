<?php
	 ...
	 * @changes    1.0.0b34  Fixed a bug in getImageType() where the fread was not reading enough bytes [jt,
	 *             2012-06-05]
	 * @changes    1.0.0b33  Fixed a method signature [wb, 2011-08-24]
	 * @changes    1.0.0b32  Added a call to clearstatcache() to ::saveChanges() to solve a bug when
	 *             Phoenix_Framework_Flourish_File::output() is called in the same script execution [wb, 2011-05-23]
	 * @changes    1.0.0b31  Fixed a bug in using ImageMagick to convert files with a colon in the filename [wb,
	 *             2011-03-20]
	 * @changes    1.0.0b30  Added a check for systems using the GD extension and no memory limit, plus a check for
	 *             ImageMagick's convert command failing [wb, 2011-03-20]
	 * @changes    1.0.0b29  Added checks for AIX [wb, 2011-01-19]
	 * @changes    1.0.0b28  Added the ::rotate() method, added code to try and prevent fatal errors due to hitting the
	 *             memory limit when using GD [wb, 2010-11-29]
	 * @changes    1.0.0b27  Backwards Compatibility Break - changed the parameter order in ::crop() from
	 *             `$crop_from_x`, `$crop_from_y`, `$new_width`, `$new_height` to `$new_width`, `$new_height`,
	 *             `$crop_from_x`, `$crop_from_y` - added `$horizontal_position` and `$vertical_position` parameters to
	 *             ::cropToRatio() [wb-imarc, 2010-11-09]
	 * @changes    1.0.0b26  Fixed a bug where processing via ImageMagick was not properly setting the default RGB
	 *             colorspace [wb, 2010-10-19]
	 * @changes    1.0.0b25  Fixed the class to not generate multiple files when saving a JPG from an animated GIF or a
	 *             TIF with a thumbnail [wb, 2010-09-12]
	 * @changes    1.0.0b24  Updated class to use Phoenix_Framework_Flourish_Core::startErrorCapture() instead of
	 *             `error_reporting()` [wb, 2010-08-09]
	 * @changes    1.0.0b23  Fixed the class to detect when exec() is disabled and the function has a space before or
	 *             after in the list [wb, 2010-07-21]
	 * @changes    1.0.0b22  Fixed ::isImageCompatible() to handle certain JPGs created with Photoshop [wb, 2010-04-03]
	 * @changes    1.0.0b21  Fixed ::resize() to allow dimensions to be numeric strings instead of just integers [wb,
	 *             2010-04-09]
	 * @changes    1.0.0b20  Added ::append() [wb, 2010-03-15]
	 * @changes    1.0.0b19  Updated for the new Phoenix_Framework_Flourish_File API [wb-imarc, 2010-03-05]
	 * @changes    1.0.0b18  Fixed a bug in ::saveChanges() that would incorrectly cause new filenames to be created,
	 *             added the $overwrite parameter to ::saveChanges(), added the $allow_upsizing parameter to ::resize()
	 *             [wb, 2010-03-03]
	 * @changes    1.0.0b17  Fixed a couple of bug with using ImageMagick on Windows and BSD machines [wb, 2010-03-02]
	 * @changes    1.0.0b16  Fixed some bugs with GD not properly handling transparent backgrounds and desaturation of
	 *             .gif files [wb, 2009-10-27]
	 ...
	 * @changes    1.0.0b13  Updated class to work even if the file extension is wrong or not present, ::saveChanges()
	 *             detects files that aren't writable [wb, 2009-07-29]
	 * @changes    1.0.0b12  Fixed a bug where calling ::saveChanges() after unserializing would throw an
	 *             Phoenix_Framework_Exception related to the image processor [wb, 2009-05-27]
	 ...
	 * @changes    1.0.0b8   Updated for new Phoenix_Framework_Flourish_Core API [wb, 2009-02-16]
	 * @changes    1.0.0b7   Changed @ error suppression operator to `error_reporting()` calls [wb, 2009-01-26]
	 * @changes    1.0.0b6   Fixed ::cropToRatio() and ::resize() to always return the object even if nothing is to be
	 *             done [wb, 2009-01-05]
	 * @changes    1.0.0b5   Added check to see if exec() is disabled, which causes ImageMagick to not work [wb,
	 *             2009-01-03]
	 * @changes    1.0.0b4   Fixed ::saveChanges() to not delete the image if no changes have been made [wb,
	 *             2008-12-18]
	 ...
	 */
	class Phoenix_Framework_Flourish_Image extends Phoenix_Framework_Flourish_File {
		// The following constants allow for nice looking callbacks to static methods
		const create = 'Phoenix_Framework_Flourish_Image::create';
		const getCompatibleMimetypes = 'Phoenix_Framework_Flourish_Image::getCompatibleMimetypes';
		const isImageCompatible = 'Phoenix_Framework_Flourish_Image::isImageCompatible';
		const reset = 'Phoenix_Framework_Flourish_Image::reset';
		const setImageMagickDirectory = 'Phoenix_Framework_Flourish_Image::setImageMagickDirectory';
		const setImageMagickTempDir = 'Phoenix_Framework_Flourish_Image::setImageMagickTempDir';
		/**
		...
		static private function checkImageMagickBinary( $path ) {
			...
			$binary = $path . ( Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) ? 'convert.exe' : 'convert' );
		 ...
		 * back.
		 *
		 * @throws Phoenix_Framework_Exception  When no image was specified or when the image already exists
		 *
		 * @param  string $file_path The path to the new image
		 * @param  string $contents  The contents to write to the image
		 *
		 * @return Phoenix_Framework_Flourish_Image
		 */
		static public function create( $file_path, $contents ) {
			if ( empty( $file_path ) ) {
				throw new Phoenix_Framework_Exception( 'No filename was specified' );
			}
			...
			$image = new Phoenix_Framework_Flourish_Image( $file_path );
			Phoenix_Framework_Flourish_Filesystem::recordCreate( $image );
		...
		static public function determineProcessor() {
			...
			if ( self::$processor === null ) {
				...
				try {
					...
					if ( Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) ) {
					...
					} elseif ( Phoenix_Framework_Flourish_Core::checkOS( 'linux', 'bsd', 'solaris', 'osx', 'aix' ) ) {
						$found = false;
						if ( Phoenix_Framework_Flourish_Core::checkOS( 'solaris' ) ) {
							$locations = array(
								'/opt/local/bin/',
								'/opt/bin/',
								'/opt/csw/bin/'
						...
						if ( ! $found && Phoenix_Framework_Flourish_Core::checkOS( 'solaris' ) ) {
							throw new Phoenix_Framework_Exception();
						}
						if ( ! $found && Phoenix_Framework_Flourish_Core::checkOS( 'linux', 'freebsd', 'aix' ) ) {
						...
						if ( ! $found && Phoenix_Framework_Flourish_Core::checkOS( 'osx', 'netbsd', 'openbsd' ) ) {
		 ...
		 *  - `'tif'`
		 *
		 * @throws Phoenix_Framework_Exception  When the file specified is not an image
		 *
		 ...
		 *
		 * @return mixed  An associative array: `'type' => {mixed}, 'width' => {integer}, 'height' => {integer}`, or
		 *                the element specified
		 */
		static protected function getInfo( $image_path, $element = null ) {
			$extension = strtolower( Phoenix_Framework_Flourish_Filesystem::getPathInfo( $image_path, 'extension' ) );
			...
			Phoenix_Framework_Flourish_Core::startErrorCapture( E_WARNING );
			$image_info = getimagesize( $image_path );
			Phoenix_Framework_Flourish_Core::stopErrorCapture();
			if ( $image_info == false ) {
				throw new Phoenix_Framework_Exception(
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When the image specified does not exist
		 *
		...
		static private function isOpenBaseDirRestricted( $path ) {
			if ( ini_get( 'open_basedir' ) ) {
				$open_basedirs = explode( ( Phoenix_Framework_Flourish_Core::checkOS( 'windows' ) ) ? ';' : ':', ini_get( 'open_basedir' ) );
				$found         = false;
		...
		/**
		 * Sets the directory the ImageMagick binary is installed in and tells the class to use ImageMagick even if GD
		 * is installed
		 *
		...
		static public function setImageMagickDirectory( $directory ) {
			$directory = Phoenix_Framework_Flourish_Directory::makeCanonical( $directory );
		...
		static public function setImageMagickTempDir( $temp_dir ) {
			$temp_dir = new Phoenix_Framework_Flourish_Directory( $temp_dir );
		 ...
		 *
		 * @throws Phoenix_Framework_Exception  When no image was specified, when the image does not exist or when the
		 *                                      path specified is not an image
		 *
		...
		public function __construct( $file_path, $skip_checks = false ) {
			...
			if ( ! self::isImageCompatible( $file_path ) ) {
				...
				throw new Phoenix_Framework_Exception(
					...
					$file_path,
					Phoenix_Framework_Flourish_Grammar::joinArray( $valid_image_types, 'or' )
				);
			}
		}
		...
		public function append( $data ) {
			throw new Phoenix_Framework_Exception( 'It is not possible to append an image' );
		}
		 ...
		 *
		 * @param  integer        $new_width   The width in pixels to crop the image to
		 * @param  integer        $new_height  The height in pixels to crop the image to
		 * @param  integer|string $crop_from_x The number of pixels from the left of the image to start the crop from,
		 *                                     or a horizontal position of `'left'`, `'center'` or `'right'`
		 * @param  integer|string $crop_from_y The number of pixels from the top of the image to start the crop from,
		 *                                     or a vertical position of `'top'`, `'center'` or `'bottom'`
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return \Phoenix_Framework_Flourish_Image The image object, to allow for method chaining
		 */
		 ...
		 * The crop does not occur until ::saveChanges() is called.
		 *
		 * @param  integer $ratio_width         The width ratio to crop the image to
		 * @param  integer $ratio_height        The height ratio to crop the image to
		...
		private function processWithGD( $output_file, $jpeg_quality ) {
			$type       = self::getImageType( $this->file );
			$save_alpha = false;
			$path_info  = Phoenix_Framework_Flourish_Filesystem::getPathInfo( $output_file );
			...
			if ( ini_get( 'memory_limit' ) != '-1' ) {
				...
				$memory_limit_bytes = Phoenix_Framework_Flourish_Filesystem::convertToBytes( ini_get( 'memory_limit' ) );
				// Estimate the memory usage and throw an Phoenix_Framework_Exception if we will run out
		 ...
		 * @param  boolean $allow_upsizing If the image is smaller than the desired canvas, the image will be increased
		 *                                 in size
		 *
		 * @throws Phoenix_Framework_Exception
		 * @return \Phoenix_Framework_Flourish_Image The image object, to allow for method chaining
		...
		public function saveChanges( $new_image_type = null, $jpeg_quality = 90, $overwrite = false ) {
			...
			if ( $new_image_type && Phoenix_Framework_Flourish_Filesystem::getPathInfo( $this->file, 'extension' ) != $new_image_type ) {
				if ( $overwrite ) {
					$path_info   = Phoenix_Framework_Flourish_Filesystem::getPathInfo( $this->file );
					$output_file = $path_info[ 'dirname' ] . $path_info[ 'filename' ] . '.' . $new_image_type;
				} else {
					$output_file = Phoenix_Framework_Flourish_Filesystem::makeUniqueName( $this->file, $new_image_type );
				}
			...
			if ( $output_file == $this->file && Phoenix_Framework_Flourish_Filesystem::isInsideTransaction() ) {
			...
			$old_file = $this->file;
			Phoenix_Framework_Flourish_Filesystem::updateFilenameMap( $this->file, $output_file );
			// If we created a new image, delete the old one
			if ( $output_file != $old_file ) {
				$old_image = new Phoenix_Framework_Flourish_Image( $old_file );
				$old_image->delete();
			}
			$this->pending_modifications = array();
			clearstatcache();
