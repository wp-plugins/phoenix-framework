<?php
	 ...
	 * @changes    1.0.0b7  Updated ::__construct() to no longer require a message, like the Exception class, and allow
	 *             for non-integer codes [wb, 2009-06-26]
	 * @changes    1.0.0b6  Fixed ::splitMessage() so that the original message is returned if no list items are found,
	 *             added ::reorderMessage() [wb, 2009-06-02]
	 * @changes    1.0.0b5  Added ::splitMessage() to replace fCRUD::removeListItems() and fCRUD::reorderListItems()
	 *             [wb, 2009-05-08]
	 * @changes    1.0.0b4  Added a check to ::__construct() to ensure that the `$code` parameter is numeric [wb,
	 *             2009-05-04]
	 * @changes    1.0.0b3  Fixed a bug with ::printMessage() messing up some HTML messages [wb, 2009-03-27]
	 * @changes    1.0.0b2  ::compose() more robustly handles `$components` passed as an array, ::__construct() now
	 *             detects stray `%` characters [wb, 2009-02-05]
	 * @changes    1.0.0b   The initial implementation [wb, 2007-06-14]
	 */
	class Phoenix_Framework_Exception extends Exception {
		/**
		...
		static public function registerCallback( $callback, $exception_type = null ) {
			if ( $exception_type === null ) {
				$exception_type = 'Phoenix_Framework_Flourish_Exception';
			}
		 ...
		 *
		 * @return integer  `-1` if `$a` is longer than `$b`, `0` if they are equal length, `1` if `$a` is shorter than
		 *                  `$b`
		 */
		 ...
		 *  - `% 2d`: Using a literal space as a padding character - a space will be used if no padding character is
		 *  specified
		 ...
		 * @throws Exception
		 *
		 * @return \Phoenix_Framework_Exception
		 */
		public function __construct( $message = '' ) {
			...
			$args = array_map( array( 'Phoenix_Framework_Exception', 'dump' ), $args );
		...
		/**
		 * Prints the backtrace to currently called exception inside of a pre tag with the class being 'exception
		 * %THIS_EXCEPTION_CLASS_NAME% trace'
		 *
		 * @return void
		 */
		 ...
		 *
		 * @param  string $match This should be a string to match to one of the list items - whatever the order this is
		 *                       in the parameter list will be the order of the list item in the adjusted message
		 * @param         string ...
		 *
		 * @return Phoenix_Framework_Exception  The exception object, to allow for method chaining
		 */
		public function reorderMessage( $match ) {
			...
			uasort( $matching_array, array( 'Phoenix_Framework_Exception', 'sortMatchingArray' ) );
		 ...
		 *
		 * @param  array $list_item_matches An array of strings to filter the list items by, list items will be ordered
		 *                                  in the same order as this array
		 * @param        array              ...
		 *
		 * @return array  This will contain an array of strings corresponding to the parameters passed - see method
