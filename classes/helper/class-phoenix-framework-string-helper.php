<?php
/**
 * class-phoenix-framework-string-helper
 *
 * a
 *
 * @class          Phoenix_Framework_String_Helper
 * @version        1.0
...
if ( ! class_exists( 'Phoenix_Framework_String_Helper' ) ) {
	class Phoenix_Framework_String_Helper {
		/**
		...
		public static function truncate( $string, $length, $suffix = '...', $encoding = null ) {
			if ( mb_strlen( $string, $encoding ? get_option( 'blog_charset' ) : null ) > $length ) {
				return trim( mb_substr( $string, 0, $length, $encoding ? get_option( 'blog_charset' ) : null ) ) . $suffix;
			} else {
				return $string;
			}
		}
		 ...
		 * @return bool
		 */
		public static function starts_with( $string, $needle ) {
